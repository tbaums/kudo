...
func MapperFunc(mgr manager.Manager) handler.ToRequestsFunc {
...
func PopulatePlanExecutionPhases(basePath string, executedPlan *kudov1alpha1.Plan, planExecution *kudov1alpha1.PlanExecution, instance *kudov1alpha1.Instance, frameworkVersion *kudov1alpha1.FrameworkVersion, configs map[string]string, recorder record.EventRecorder) error {
				...
				if taskSpec, ok := frameworkVersion.Spec.Tasks[t]; ok {
					...
					for _, res := range taskSpec.Resources {
						if resource, ok := frameworkVersion.Spec.Templates[res]; ok {
							...
							if err != nil {
								recorder.Event(planExecution, "Warning", "InvalidPlanExecution", fmt.Sprintf("Error expanding mustache: %v", err))
...
func ParseConfig(planExecution *kudov1alpha1.PlanExecution, instance *kudov1alpha1.Instance, frameworkVersion *kudov1alpha1.FrameworkVersion, recorder record.EventRecorder) (map[string]string, error) {
	//Load parameters:
